...
impl Renderer<'_> for CpuRenderer<'_> {
    fn render(&mut self, view_state: &ViewState, light: &Light) {
        ...
        for pixel in &self.pixels {
            let (sx, sy, cx, cy) = (view_state.angle_x.sin(), view_state.angle_y.sin(), view_state.angle_x.cos(), view_state.angle_y.cos());
            ...
            let projected = Operations::project(view_state.scale, view_state.focal_distance, self.canvas_width as f32, self.canvas_height as f32, rotated.0, rotated.1, rotated.2, view_state.perspective_factor);
            let block_size = (view_state.scale / (view_state.focal_distance + rotated.2) * pixel.size_factor).ceil() as u32;
